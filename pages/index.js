...
const STAGE_CRITERIA = {
  contact: ["greeting", "rapport", "energy", "opening", "interest"],
  needs: ["questions", "listening", "empathy", "discovery", "summary"],
  presentation: ["benefits", "examples", "clarity", "value", "relevance"],
  objections: ["joining", "questions", "arguments", "price", "closing"],
  closing: ["signal", "offer", "urgency", "confirmation", "nextStep"],
};

const STAGE_CRITERIA_LABELS = {
  contact: ["Приветствие", "Расположение", "Энергия", "Открытие диалога", "Вызов интереса"],
  needs: ["Вопросы", "Слушание", "Эмпатия", "Выявление боли", "Резюмирование"],
  presentation: ["Выгоды", "Примеры", "Ясность", "Ценность", "Релевантность"],
  objections: ["Присоединение", "Уточнение", "Аргументы", "Работа с ценой", "Закрытие"],
  closing: ["Сигнал покупки", "Предложение", "Срочность", "Подтверждение", "Следующий шаг"],
};
      ...
      <div style={{ background: '#0f172a', borderBottom: '1px solid #1e293b', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        {(step === 'chat' || step === 'evaluating') && (
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 12, color: '#475569' }}>{nicheData?.emoji} {nicheData?.label}</span>
            <span style={{ fontSize: 12, color: '#3b82f6' }}>· {stageData?.emoji} {stageData?.label}</span>
